...
1. **자동 스캔**: 지원 LLM 사이트에서 새 어시스턴트 답변이 렌더링되면 본문 텍스트를 추출해 **Vercel에 배포된 Truth History 백엔드**(`/api/v1/scan/text`)로 전송합니다. **답변 내 이미지도 자동 검증**되어 각 이미지 하단에 점수·근거 배지가 추가로 삽입됩니다.
...
3. **수동 검사**: 어떤 텍스트든 드래그 선택 후 우클릭 → "Truth History: 이 텍스트 역사 할루시네이션 검사"로 즉시 검증할 수 있습니다. **이미지 우클릭 → "이 이미지 위변조(합성) 검사"**, **영상 우클릭 → "이 영상 딥페이크 검사"**도 모든 사이트에서 동작합니다.
