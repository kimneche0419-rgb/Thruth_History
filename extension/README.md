...
1. **자동 스캔**: 지원 LLM 사이트에서 새 어시스턴트 답변이 렌더링되면 본문 텍스트를 추출해 **Vercel에 배포된 Truth History 백엔드**(`/api/v1/scan/text`)로 전송합니다. **답변 내 이미지는 전부 자동 검증**되어 각 이미지 하단에 점수·근거 배지가 삽입되며, **YouTube 영상 링크·임베드도 자동 검증**됩니다.
2. **XAI 배지 삽입**: 각 답변 상단에 신뢰도·위험도·판정 근거(이유 목록) 배지를 인라인으로 삽입합니다. 배지는 **투명 배경 + 상속 글자색**으로 호스트 페이지의 라이트/다크 테마에 자동 적응하며, 판정 태그는 위험도와 연동된 3단계(정상/판정 보류·주의/왜곡 의심)로 표시됩니다. **배지를 클릭하면 상세 리포트 패널**(신뢰도·AI 생성 확률 + 판정 근거 + 근거 자료 웹사이트 클릭 가능 링크 + 참고 사료)이 열립니다.
3. **수동 검사**: 어떤 텍스트든 드래그 선택 후 우클릭 → "Truth History: 이 텍스트 역사 할루시네이션 검사"로 즉시 검증할 수 있습니다. **이미지 우클릭 → "이 이미지 위변조(합성) 검사"**, **영상(YouTube 링크·임베드·재생 프레임) 우클릭 → "이 영상 딥페이크·왜곡 검사"**도 모든 사이트에서 동작합니다.
...
> **영상 검증은 YouTube 기반**입니다. YouTube URL(`watch`·`embed`·`shorts`·`youtu.be`)을 oEmbed 메타데이터로 해석해 ① 썸네일 이미지 합성 분석(`/api/v1/scan/media`)과 ② 제목·채널명 텍스트 고증(`/api/v1/scan/text`)을 병렬 검증한 뒤, 신뢰도는 최솟값·위험도는 최악값·근거는 통합하여 리포트로 제공합니다. YouTube가 아닌 직접 파일 URL은 `/api/v1/scan/media`로 검증합니다.
...
* `host_permissions`(`http://*/*`, `https://*/*`): 모든 사이트 우클릭 수동 검사(텍스트·이미지·영상) 및 배포 백엔드(Vercel)·검증 대상 미디어 다운로드용.
